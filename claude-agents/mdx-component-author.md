...
description: |
  ...
  `components/mdx/index.ts`, and registers it in `mdx-components.tsx`.
...
3. `components/mdx/` — look at `Callout.tsx`, `Hardware.tsx`, `Task.tsx`, `MiniProject.tsx` to match style: server components, Polish labels, Tailwind v4 utility classes only.
...
- **JSDoc at the top.** Document props with a short example invocation, so content authors can read it.
